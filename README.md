<h1 align="center">
  Olá, mundo! Sou Allan Eric Jepsen 👋
</h1>

<h3 align="center">
  Data Scientist · Data Engineer · AI/LLM Builder · Mulesoft Consultant
</h3>

<p align="center">
  <a href="https://aejepsen.github.io" target="_blank">
    <img src="https://img.shields.io/badge/Site_Pessoal-0e1018?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/allan-ericjepsen" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://www.youtube.com/@aejepsen" target="_blank">
    <img src="https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white"/>
  </a>
  <a href="https://www.instagram.com/aejepsen" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=16&pause=1000&color=00E5C8&center=true&vCenter=true&width=600&lines=Construindo+agentes+de+IA+com+LangGraph+%26+Claude+API;Data+Engineer+%7C+Apache+Spark+%7C+Neo4j+%7C+Airflow;MBA+Data+Science+%26+Analytics+%E2%80%94+USP%2FEsalq;AWS+Architect+Associate+%7C+Azure+Data+Scientist;50dev%2B+%F0%9F%9A%80+Litoral+Paulista+%E2%80%94+Brasil" alt="Typing SVG" />
</p>

---

## 🚀 Sobre mim

- 💼 **Consultor de Soluções Digitais** na **Capgemini**
- 🤖 Construo **agentes de IA** com RAG, LangGraph, Claude API e Mem0
- 📊 **Data Engineer** — Apache Spark, Airflow, Neo4j, pipelines ETL/ELT
- 🎓 **MBA Data Science & Analytics** — USP/Esalq · **Bacharelado em Ciências de Dados** — Univesp
- ☁️ **AWS Solutions Architect Associate** · **Microsoft Azure Data Scientist Associate (AI-900 · AZ-900 · DP-900)**
- 🌊 Moro no **litoral paulista** · Mineiro de coração 🧡
- 🙏 *"E como vós quereis que os homens vos façam, da mesma maneira lhes fazeis vós também"* — Lc 6:31
- 🎂 **50dev+** e contando...

---

## 🛠️ Stack Técnico

### 🤖 IA & LLMs
![Claude API](https://img.shields.io/badge/Claude_API-D97757?style=for-the-badge&logo=anthropic&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-00E5C8?style=for-the-badge&logoColor=black)
![Mem0](https://img.shields.io/badge/Mem0_Memory_2.0-7B61FF?style=for-the-badge&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-D97757?style=for-the-badge&logo=anthropic&logoColor=white)
![Qdrant](https://img.shields.io/badge/Qdrant-DC244C?style=for-the-badge&logoColor=white)

### 🔬 Data Science & ML
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![YOLO](https://img.shields.io/badge/YOLO-00FFFF?style=for-the-badge&logoColor=black)

### ⚙️ Data Engineering
![Apache Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=for-the-badge&logo=apacheairflow&logoColor=white)
![Neo4j](https://img.shields.io/badge/Neo4j-008CC1?style=for-the-badge&logo=neo4j&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### ☁️ Cloud & Integração
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Microsoft_Azure-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)
![Mulesoft](https://img.shields.io/badge/Mulesoft-00A1DF?style=for-the-badge&logo=mulesoft&logoColor=white)
![Power Automate](https://img.shields.io/badge/Power_Automate-0066FF?style=for-the-badge&logo=microsoftpowerautomate&logoColor=white)

### 💻 Dev & Frontend
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![C#](https://img.shields.io/badge/C%23-.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)

---

## 🌟 Projetos em Destaque

### 🏢 Capgemini

| Projeto | Descrição | Stack |
|--------|-----------|-------|
| **AI Professor** | Agente de conhecimento corporativo com RAG, memória persistente (Mem0) e orquestração via LangGraph | Claude API · LangGraph · Qdrant · Supabase · Mem0 |
| **Automação de Planilhas** | Pipeline Python/Pandas para consolidação automática de múltiplas planilhas, eliminando retrabalho manual | Python · Pandas · ETL |
| **IA Generativa + Mulesoft** | Integração de IA generativa em aplicação Mulesoft para gestão empresarial via MCP, Claude e Postgres | Mulesoft · MCP · Claude · PostgreSQL |

### 🚀 Empreendedor / Projetos Próprios

| Projeto | Descrição | Stack |
|--------|-----------|-------|
| **Laudifier** | Gerador de laudos médicos com RAG, entrada por voz (PT-BR), exportação PDF/DOCX e memória com Mem0 2.0 | Claude · LangGraph · Qdrant · Whisper · Mem0 |
| **SuaSaladaTech** | Sistema IoT com ESP32 para controle de horta hidropônica indoor — temperatura, pH, luminosidade em tempo real | Node-RED · ESP32 · MicroPython |

### 📊 Data Engineering / ML

| Projeto | Descrição | Stack |
|--------|-----------|-------|
| **Instagram → Neo4j Pipeline** | Pipeline completo do CSV ao banco de grafos com Explain Profile e SKILL para Agente Claude | Neo4j · Cypher · Python · Pandas |
| **Churn Prediction** | Pipeline Spark com Random Forest para detecção de churn em escala distribuída | Apache Spark · Scikit-learn · Python |
| **Airflow + Postgres Pipeline** | Orquestração Airflow com migração entre bancos e exportação CSV automatizada | Airflow · PostgreSQL · Docker |
| **YOLO — Doenças em Café** | Visão computacional para detecção de doenças fúngicas em folhas de café | YOLO · Deep Learning · Python |

---

## 🏅 Certificações

<p>
  <img src="https://img.shields.io/badge/AWS-Solutions_Architect_Associate-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/Azure-AI--900-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
  <img src="https://img.shields.io/badge/Azure-AZ--900-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
  <img src="https://img.shields.io/badge/Azure-DP--900-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
  <img src="https://img.shields.io/badge/Azure-SQL-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
  <img src="https://img.shields.io/badge/Cybersecurity-Threat_Vectors-222222?style=for-the-badge&logo=hackthebox&logoColor=white"/>
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=aejepsen&show_icons=true&theme=gotham&hide_border=true&count_private=true" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=aejepsen&layout=compact&theme=gotham&hide_border=true&langs_count=8" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=aejepsen&theme=gotham&hide_border=true" />
</p>

---

## 🎓 Formação

| Instituição | Curso | Período |
|------------|-------|---------|
| **USP / Esalq** | MBA Data Science & Analytics | 2023 – 2025 |
| **Univesp** | Bacharelado em Ciências de Dados | 2021 – 2025 |
| **Trybe** | Desenvolvimento Web Fullstack | 2021 – 2023 |
| **FGV** | MBA Finanças Corporativas | 2008 – 2009 |

---

## 📬 Contato

<p align="center">
  <a href="mailto:aejepsen@yahoo.com.br">
    <img src="https://img.shields.io/badge/Email-aejepsen@yahoo.com.br-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/aejepsen?tab=repositories" target="_blank">
    <img src="https://img.shields.io/badge/Repositórios-GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=aejepsen&color=00e5c8&style=flat-square&label=Visitas+ao+perfil" />
</p>

<p align="center"><i>"E como vós quereis que os homens vos façam, da mesma maneira lhe
